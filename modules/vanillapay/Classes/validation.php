...
class AriaryNetIPN extends AriaryNet
{
    ...
    public function getIPNTransactionDetails()
    {
        ...
        $msgpusher="Niditra";
        $pusher->trigger('channel_tout', 'notification_tout', array('message' => $msgpusher));
        $pusher->trigger('channel_tout', 'notification_tout', array('message' => $id_paiement));
        ...
        $pusher->trigger('channel_tout', 'notification_tout', array('message' => $json));
        ...
        if ($resultat == "success") {
            ...
            //$this->module->validateOrder((int)$idpanier, Configuration::get('PS_OS_PAYMENT'), $montant, $this->module->displayName, null, array(), (int)$currency->id, false, $customer->secure_key);
